...
const PlanNetworkMapFieldsTable: FC<PlanNetworkMapFieldsTableProps> = ({
  ...
  const { control, trigger, watch } = useFormContext<PlanNetworkEditFormValues>();

  const networkMappings = watch(NetworkMapFieldId.NetworkMap);
  const {
    append,
  ...
  // validate initial values on first load
  useEffect(() => {
    setTimeout(async () => {
      await trigger();
    }, 0);
