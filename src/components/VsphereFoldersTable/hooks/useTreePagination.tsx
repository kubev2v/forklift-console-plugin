...
const useTreePagination = ({
  ...
  return useMemo(() => {
    ...
    for (let blockIndex = 0; blockIndex < blocks.length; blockIndex += 1) {
      ...
      const folderRow = block.folder;
      if (!folderRow?.isHidden) {
    ...
    if (!isEmpty(pageDescriptors) && pageDescriptors[0].type === ROW_TYPE.Vm) {
      ...
      pagedRows.push(parentBlock.folder); // sticky header (not counted)
    }

    for (const descriptor of pageDescriptors) {
      if (descriptor.type === ROW_TYPE.Folder) {
        pagedRows.push(blocks[descriptor.blockIndex].folder);
      } else {
