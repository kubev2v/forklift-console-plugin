...
const VsphereUrlField: FC = () => {
  ...
  const [endpointType, certificateValidation] = useWatch({
    control,
    name: [ProviderFormFieldId.VsphereEndpointType, ProviderFormFieldId.CertificateValidation],
  });
  ...
  const {
    ...
    control,
    name: ProviderFormFieldId.VsphereUrl,
    rules: {
  ...
  return (
    ...
    >
      <TextInput
        id={ProviderFormFieldId.VsphereUrl}
        type="text"
        value={value ?? ''}
        onChange={(_event, val) => {
          onChange(val);
