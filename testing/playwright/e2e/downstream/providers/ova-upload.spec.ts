...
import type { TestProvider } from '../../../fixtures/helpers/resourceCreationHelpers';
...
import { testError, testLog } from '../../../utils/testLog';
...
test.describe('OVA Provider Upload Tests', { tag: '@downstream' }, () => {
  ...
  test.skip('should create OVA provider with applianceManagement enabled and validate upload form', async ({
    ...
    let ovaProvider: TestProvider | null = null;
    ...
    await test.step('Navigate to provider details page', async () => {
      if (!ovaProvider) {
        throw new Error('OVA provider was not created');
      }
    ...
    await test.step('Upload OVA file and verify success', async () => {
      if (!ovaProvider?.testData?.hostname) {
        throw new Error('OVA provider test data is missing hostname');
      }
      ...
      nfsCleanupManager.addOvaFileFromUrl(uniqueOvaFileName, ovaProvider.testData.hostname);

      // Wait for either success or error alert
      const { errorAlert, successAlert } = providerDetailsPage.detailsTab;
    ...
    await test.step('Upload same OVA file again and verify error', async () => {
      ...
      await expect(providerDetailsPage.detailsTab.errorAlert).toContainText(/already exists/iu);
    });
    ...
    await test.step('Verify uploaded VM appears in Virtual Machines tab', async () => {
      ...
      testLog('Verifying VM exists on NFS server:', vmName);
