...
echo "Creating project with title \"[Forklift $VERSION] UI Localization - Sprint $SPRINT/Branch $BRANCH\""

echo "Exporting PO files"
npm run export-pos
echo "Exported all PO files"

PROJECT_INFO=$(memsource project create --name "[Forklift $VERSION] UI Localization - Sprint $SPRINT/Branch $BRANCH" --template-id 169304 -f json)
