...
const messages = require('../locales/en/plugin__forklift-console-plugin.json');
...
TARGETS_FOR_DUMMY_LOCALE.forEach((target) => {
  const destination = path.join('locales', target, 'plugin__forklift-console-plugin.json');
  insertDummyLocaleAndSave(messages, destination, wrappers[target]);
  console.log(`[i18n-scripts] dummy locale for ${target} inserted to ${destination}`);
});
